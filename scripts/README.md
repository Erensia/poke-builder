...
## battle-ai-scenarios.mjs — `npm run test:ai`

배틀 AI·엔진 규칙 시나리오(ver.1.7). 특정 상황을 직접 만들어 규칙 하나하나를 확인한다 — 부유·여왕의위엄·
기합의띠·하드 오버라이드·명중률·메가진화·유턴류·변화기(상태이상·벽·설치기·날씨·트릭룸·앙코르 등)·방어류 묶음별
판단·엔진 날씨 규칙·공격기 랭크 변화·매치업 난수별 데미지 대조. 실패가 하나라도 있으면 exit 1.
브라우저 없이 Vite `ssrLoadModule`로 `src/`의 실제 코드를 불러온다.

## battle-ai-sim.mjs — `npm run sim:ai -- <모드> [판 수] [파라미터 JSON] [상대 파라미터 JSON]`

시드 고정 무작위 파티(3마리) 배틀을 실제 엔진으로 자동 진행한다.

| 모드 | 내용 |
|---|---|
| `regress` | 무작위 행동끼리 배틀 후 전체 로그 해시 — **엔진 수정 전후 해시가 같으면 동작 무변경** |
| `ai` | AI 대 무작위 봇·AI 대 AI 승률, 턴당 판단 시간, NaN 점수 수 |
| `greedy` | AI 대 그리디 봇(교체 없이 가장 빨리 잡는 기술만), 파티 좌우 교대 — 파라미터 튜닝 기준 |
| `h2h` | AI(파라미터 A) 대 AI(파라미터 B) — 상대 모델처럼 그리디 봇이 편향되는 변경의 비교용 |
