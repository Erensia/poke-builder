...
별도로 **포챔스 입국몬 전종 데이터 추가**(§6)는 위 1~4단계와 독립적인 **상시 작업**이다. 사용자가 세대별로 포켓몬 데이터 JSON을 작성 중이며, 한 세대분이 완성될 때마다 그 세대 포켓몬을 `pokemon.json`에 반영한다. 코드 페이즈 진행과 무관하게 세대 단위로 커밋한다.
...
- [ ] **포챔스 입국몬 전종 데이터 추가**(§6) — 상시. 사용자가 세대별 JSON 작성 중, 한 세대분 완성될 때마다 반영. 현재 로스터(52종)는 제외.
