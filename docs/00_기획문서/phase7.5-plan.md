...
- **신규 기술 5종** (수치·설명·태그·PP 전부 2026-09-02 사용자 확정):
  ...
  - **[후속 완료] 반동-랭크 자기 하락 누락 보강**(2026-09-02, 커밋 `10364fa`): `인파이트`·`아머캐논`(방어·특방 -1) · `엄청난힘`(공격·방어 -1) · `리프스톰`·`오버히트`·`용성군`(특공 -2) · `암해머`(스피드 -1) — effect엔 명시돼 있으나 `statChanges` self가 비어 자기 하락이 안 걸리던 버그. 본가 수치대로 채움. 스모크 8종 PASS.
...
- **파밀리쥐**: 야생 변이(네 식구 75% / 세 식구 25%)는 몸무게만 다름 → 더 흔한 **네 식구**(2.8kg) 기준. **돌핀맨**: 마이티체인지로 배틀 중 나이브폼→마이티폼 영구 변신 메커니즘 → 포획값인 **나이브폼**(총합 457) 기준. 마이티폼(HP100/공160/방97/특공106/특방87/스피드100, 총합 650)은 **폼체인지 로직 배선 시 별도 반영 예정(TODO — 사용자 확정)**.
