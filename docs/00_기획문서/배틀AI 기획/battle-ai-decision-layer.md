...
| 명령 | 내용 |
|---|---|
| `npm run test:ai` | 규칙 시나리오 24건(첫 턴 전용 기술 2건 포함, 부유·여왕의위엄·기합의띠·하드 오버라이드·명중률·메가진화·유턴류 + §4-1 변화기: 도깨비불·전기자석파·리플렉터·스텔스록 재평가/이월, 실패 판정, 이기는 대면에서 공격 유지, 배턴터치 등). 실패 시 종료 코드 1 |
