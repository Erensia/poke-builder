# 레귤레이션 M-C 스테이징 (아카이브 — 병합 완료)

- **출처**: 사용자 제공 `C:\Users\kyongmin.kim\Downloads\M-C\` (2026-09-09)
- **상태**: ✅ **완료.** `src/data/*.json`에 전부 병합됨(PR #53~#64) + 엔진 트랙(신규 기술 6종·
  특성 6훅·풀모피·지닌도구 12종, PR #66~#73)까지 배선 완료. dev→main 반영 완료(ver.1.2).
  이 폴더는 더 이상 활성 작업 대상이 아니라 병합 과정을 기록해둔 참고용 아카이브다
  (`docs/02_.../_archive/`로 이동, 2026-09-14).
- **시행일**: 2026-09-09~2026-09-11(병합 완료)

아래 내용은 병합 **전** 시점(2026-09-09)의 스냅샷 — "남은 작업"·"드라이런 결과" 섹션은
전부 과거형으로 읽을 것(당시엔 남아있었지만 지금은 전부 끝났다).
