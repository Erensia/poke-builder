...
export interface Move {
  ...
  selfFaints?: boolean;
  /**
   * 유턴·볼트체인지·톤보가에리처럼, 명중해서 효과를 준 뒤(대상 기절 여부 무관) 사용측이 교대
   * 포켓몬과 교체하는 기술만 채운다. runTurn이 이 플래그가 붙은 행동의 성공을 확인하면
   * RunTurnOutcome.selfSwitch에 그 편을 실어, 호출부가 교체 슬롯을 고르게 한다. 빗나감·행동불능·
   * 완전 무효(0배)면 교체하지 않고, 교대할 슬롯이 없어도 교체 없이 그 턴이 끝난다.
   */
  selfSwitchAfterDamage?: boolean;
  /**
   * 배턴터치 전용. selfSwitchAfterDamage와 같은 자체 교체이되, 능력 랭크 변화·급소 랭크·
   * 대타·멸망 카운트·일부 지속 volatile(혼란·씨뿌리기·뿌리박기·아쿠아링·시럽범벅)까지 교대
   * 포켓몬이 그대로 이어받는다. 데미지가 없는 변화기라 필중이면 항상 성공한다.
   */
  passesStatsOnSelfSwitch?: boolean;
  /**
   * 꼬리자르기 전용. 자신의 최대 HP 1/2을 깎아 그만큼의 대타를 세운 뒤 교대 포켓몬과 교체하고,
   * 세운 대타는 새로 나온 포켓몬이 이어받는다. HP가 절반 이하이거나, 이미 대타가 있거나, 교대할
   * 살아있는 예비가 없으면 실패한다(대타도 안 세우고 교체도 안 함). 대타 세팅이 성공했을 때만
   * 유턴류와 같은 교체 일시정지(RunTurnPaused)로 넘어간다.
   */
  shedTail?: boolean;
  /**
   * 썰렁개그 전용. 기술을 쓰기만 하면(행동불능 등으로 아예 못 쓴 경우 제외) 기술 효과의 성공 여부와
   * **무관하게** 사용 후 교대 포켓몬과 교체한다 — 이미 눈이라 날씨 설정이 실패해도 교체는 한다(사용자
   * 확인). 유턴류(selfSwitchAfterDamage)는 효과가 실패하면 교체도 안 하는 것과 다르다. 로그에 "○○은(는)
   * 트레이너의 곁으로 돌아간다!"가 붙는다.
   */
  selfSwitchAfterUse?: boolean;
  /**
   * 썰렁개그: 기술을 실제로 쓸 때 "○○의 썰렁개그" 줄 **앞에** "○○은(는) …" 형태로 붙는 대사(문구 뒷부분만 —
   * 예: "썰렁한 개그를 선보였다!"). 턴 시작 시 안내하는 turnStartUserAnnouncement(부리캐논)와 달리 행동 시점에
   * 나오고, 행동불능으로 못 썼으면 나오지 않는다.
   */
  preUseUserAnnouncement?: string;
  /**
   * 드래곤테일·배대뒤치기·울부짖기·날려버리기 — 명중 시 **상대(대상)**를 파티의 살아있는 예비
   * 포켓몬 중 무작위 1마리로 강제 교체시킨다. 데미지 기술(드래곤테일·배대뒤치기)은 데미지를 준
   * 뒤 교체하고, 타입 면역(0배)이면 교체도 일어나지 않는다. 변화기(울부짖기·날려버리기)는
   * 필중·데미지 없이 교체만 한다. 우선도 -6이라 보통 상대가 이미 움직인 뒤 발동한다. 상대에게
   * 살아있는 예비가 없으면(=파티 1마리) 교체는 생략된다 — 데미지 기술은 데미지만 정상 적용.
   * 흡반(preventsForcedSwitch)·뿌리박기(ingrain)면 밀려나지 않고, 울부짖기는 방음(소리)에 막힌다.
   * 파티 개념이 있는 배틀타워에서만 의미가 있다(1v1 매치업 스냅샷엔 영향 없음).
   */
  forcesTargetSwitch?: boolean;
  /**
  ...
  /**
   * 비바라기·쾌청·모래바람·설경·썰렁개그처럼 날씨를 바꾸는 기술만 채운다. 다른 날씨가 있으면 덮어쓰고,
   * **이미 같은 날씨면 실패한다**(필드·트릭룸과 같은 규칙, 남은 턴도 다시 채우지 않음 — 본가 규칙,
   * 사용자 확인). 지속시간은 기본 5턴, 사용자가 그 날씨에 맞는 바위(Item.weatherDurationBonus)를 지녔으면 8턴.
   */
