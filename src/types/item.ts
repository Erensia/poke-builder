...
export interface Item {
  ...
  /**
   * 멘탈허브: 헤롱헤롱·도발·사슬묶기·앙코르 중 하나가 걸리는 순간 자동으로 풀고 소모된다.
   * 본가 설명 텍스트는 연속불가·회복봉인도 포함한 6개 상태를 언급하지만, 이 두 상태를 거는
   * 기술 자체가 이 로스터에 없어(대상 없음) 실제로 걸릴 수 있는 4개만 대상으로 삼는다.
   */
  curesMentalVolatilesOnInflict?: boolean;
  /**
  ...
  /** 아름다운허물(트랙 M6): 교체 봉쇄(검은눈빛·블록·문어굳히기·물고버티기·페어리록)를 무시하고 교체할 수 있다 */
  escapesTrapping?: boolean;
