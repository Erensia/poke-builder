 ...
 *  - saltCure(소금절이): 소금절이에 맞으면 배틀이 끝날 때까지(leechSeed처럼 턴 카운터 없이) 유지된다.
 *    매 턴 종료 시 최대 HP 1/16을 잃고, 걸린 쪽이 강철 또는 물 타입이면 1/8을 잃는다(매직가드면
 *    데미지 면제). 인분(blocksSecondaryEffects)·우격다짐(tradesSecondaryEffectForPower)에는 부가효과
 *    취급이라 아예 걸리지 않는다.
