...
- 포켓몬 44종 (메가진화 가능 22종, 그중 리자몽은 메가X/Y 2폼)
- 기술 394개 (우선도·랭크 변화·다단히트 정보 포함)
- 특성 80개 (전부 효과 설명 포함, 그중 다수는 결정력/내구력/배틀 로직에 실제 반영되도록 구조화)
...
- [docs/phase6.5-plan.md](docs/phase6.5-plan.md) — Phase 6.5 기획(완료): 매치업(결정력 & 내구력) 페이지에 "이전 턴 가정" 토글 + 스피드 비교 + 날씨/필드/스크린 축 신설, 대전 로그 2차 테스트 8건(가속·탈·지구력·스텔스록·랭크 상한/결과 문구·방어 vs 랭크업·실능치 즉시 반영) — §4 로그 문구 전면 정비·§8 매직미러는 다음 페이즈로 이월
...
- ✅ Phase 6.5: 매치업(결정력 & 내구력) 페이지 개선 — "이전 턴 가정" 토글(도구 강탈·곡예 스피드 2배·성묘 배율), 스피드 비교 신설(트릭룸·마비 가정 포함), 날씨 배경·필드 선택·스크린(리플렉터/빛의장막/오로라베일) 축 신설, 부자유친·다단히트 결정력 반영; 대전 로그 2차 테스트 8건(가속 첫 턴·탈 문구·지구력 문구·스텔스록 설치·랭크 상한/결과 문구·방어 vs 랭크업·실능치 즉시 반영) ([docs/phase6.5-plan.md](docs/phase6.5-plan.md))
