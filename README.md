...
- 포켓몬 48종 (메가진화 가능 24종, 그중 리자몽은 메가X/Y 2폼)
- 기술 398개 (우선도·랭크 변화·다단히트 정보 포함)
- 특성 89개 (전부 효과 설명 포함, 그중 다수는 결정력/내구력/배틀 로직에 실제 반영되도록 구조화)
- 도구 99종 (메가스톤 포함, 전부 효과 설명 포함. 다수는 데미지 위력 보정 배율로 구조화)
- 성격 21종
...
- [docs/phase7-plan.md](docs/phase7-plan.md) — Phase 7 기획: 배틀타워(구 대전 로그) 2차 테스트 수정사항(문구 표준화 + 무적 상태·무릎차기·철제광선 로직 버그 + 미러코트·기사회생·죽기살기 구현 + 모래바람 틱 + 미러아머 확장 + 빌더 검색 UI), 곁다리 특성 로직(복안·페어리오라·매직가드·매직미러·인분 완료), 로스터 확장(플라엣테·픽시)
- [docs/phase7.5-plan.md](docs/phase7.5-plan.md) — Phase 7.5 기획: (2026-09-01 재편) 선행 UI 정리 3건(기술표 열 정렬·난수 격파 확률·특수 데미지 로직 감사)은 완료, 배틀타워 리뉴얼은 Phase 8로 분리. **현재 스코프 = 포켓몬 로스터 확장(포챔스 입국몬 전체)** — 세대별 JSON을 순차 병합, 현 로스터 52종 제외
- [docs/phase8-plan.md](docs/phase8-plan.md) — Phase 8 기획: 배틀타워 리뉴얼 — 파티 선출·교체 인프라(파티 배열 BattleState·교체 액션·등장 시 처리 파이프라인), 상대 진영 설치 기술 6종, 플라워베일·공생, 1대1 우회 복원(가속·곡예·씨뿌리기 등). 구 phase7.5-plan §4에서 분리
...
- 🔲 Phase 7: 곁다리 특성 로직 배선(복안·페어리오라·매직가드·매직미러·인분) + 로스터 확장(플라엣테·픽시, 48종) 완료; 배틀타워(구 대전 로그) 2차 테스트 수정사항 진행 중 — 문구 표준화, 무적 상태·무릎차기·철제광선 로직 버그, 미러코트·기사회생·죽기살기 구현, 모래바람 틱 데미지, 미러아머 반사 확장, 파티 빌더 검색 UI ([docs/phase7-plan.md](docs/phase7-plan.md))
- 🔲 Phase 7.5: 포켓몬 로스터 확장 — 포챔스 입국몬 전체를 세대별 JSON으로 순차 병합(현 로스터 52종 제외). ※ 선행 UI 정리 3건(기술표 열 정렬·난수 격파 확률 표기·특수 데미지 로직 감사)은 완료 ([docs/phase7.5-plan.md](docs/phase7.5-plan.md))
- 🔲 Phase 8: 배틀타워 리뉴얼 — `phase-8` 브랜치 구현 완료(2026-09-03). 편측 3마리 파티(슬롯 0 리드)·교체 인프라(`BattleState.sideA/sideB` 파티 배열·`move｜switch` 액션·`performSwitch` 등장/퇴장 파이프라인·강제 교체), 파티 트래커·"기술/교체" 토글 UI, 상대 진영 설치 기술 6종(스텔스록·압정뿌리기·독압정·끈적끈적네트·비검천중파·암석액스 — 스택·등장 데미지·제거·독타입 흡수), 플라워베일·공생, 1대1 우회 복원(가속 자발 교체 억제·곡예/씨뿌리기류 교체 해제). 문구 다듬기·6슬롯 선출·메가진화 버튼·일루전은 post-1.0 백로그 ([docs/phase8-plan.md](docs/phase8-plan.md))
