...
- [docs/phase6.5-plan.md](docs/phase6.5-plan.md) — Phase 6.5 기획(완료): 매치업(결정력 & 내구력) 페이지에 "이전 턴 가정" 토글 + 스피드 비교 + 날씨/필드/스크린 축 신설, 대전 로그 2차 테스트 8건(가속·탈·지구력·스텔스록·랭크 상한/결과 문구·방어 vs 랭크업·실능치 즉시 반영), 로스터 마무리(비비용, 46종) — §4 로그 문구 전면 정비·§8 매직미러는 다음 페이즈로 이월
...
- 🔲 Phase 7(예정): 대전 로그 UI 문구 전면 정비(§4 이월), 매직미러·진영 설치 기술 스택/제거(§8, 파티 선출·교체 도입 후)
